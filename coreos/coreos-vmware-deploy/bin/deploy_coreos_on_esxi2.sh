...
update_user_data(){
  ...
  if ! which genisoimage > /dev/null 2>&1; then
	echo "Error: genisoimage does not exist on your system"
	exit 1
  fi
  ...
  genisoimage -R -input-charset utf-8 -V config-2 -o ${CLOUD_CONFIG_ISO} ${TMP_CLOUD_CONFIG_DIR}
# genisoimage -R -input-charset utf-8 -V config-2 -o ${CLOUD_CONFIG_ISO} ${TMP_CLOUD_CONFIG_DIR}
...
echo "Waiting 30 seconds for power.on"
sleep 30
...
#      UPNAME=${UPCASE#--}  #remove --
      ...
      UPCASE=$(echo $key |  tr [a-z] [A-Z]) #upcase
      UPNAME=${UPCASE#--}  #remove --
